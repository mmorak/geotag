...
public class GPSBabelTask extends BackgroundTask<Gpx> {
  ...
  /**
   * A utility class reading the output of GPSBabel and
   * generating progress or error messages
   * @author Andreas Schneider
   *
   */
          ...
          if (b == -1)
            ...
            if (b == '\r' || b == '\n' || bytesInBuffer == buffer.length) {
              ...
              while (bytesInBuffer > 0
                && (buffer[bytesInBuffer - 1] == '\r'
                  || buffer[bytesInBuffer - 1] == '\n')) {
                bytesInBuffer--;
              }
    ...
    public void handleGPSBabelLine(String line) {
      ...
      try {
        ...
        if (tokenizer.countTokens() == 3) {
          ...
          firePropertyChange(
              "progress", progressMessage, message); //$NON-NLS-1$
