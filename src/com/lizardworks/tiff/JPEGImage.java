...
class JPEGImage extends CodedImage {
   ...
   * len+=imageStrips[i].length; imageBytes = new byte [len]; for (i=0,n=0; i<imageStrips.length;
   * i++) { System.arraycopy(imageStrips[i], 0, imageBytes, n,
   * imageStrips[i].length); n += imageStrips[i].length; } imageStrips = null; }
   * img = Toolkit.getDefaultToolkit().createImage(imageBytes); return img; }
   */
